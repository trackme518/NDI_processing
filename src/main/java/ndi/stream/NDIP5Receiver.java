 ...
 */
public class NDIP5Receiver extends NDIP5FrameCleaner implements AutoCloseable {
    /**
     ...
     * With obscure color formats, the color format of each frame received should be checked with {@link NDIP5VideoFrame#getFourCCType()}
     *
     ...
     * @param colorFormat The color format to transcode received frames to. It is recommended to use {@link NDIP5Receiver.ColorFormat#FASTEST}
     * @param receiveBandwidth The bandwidth to receive video at. This can be selected from {@link NDIP5Receiver#RECEIVE_BANDWIDTH_METADATA_ONLY}, {@link NDIP5Receiver#RECEIVE_BANDWIDTH_AUDIO_ONLY}, {@link NDIP5Receiver#RECEIVE_BANDWIDTH_LOWEST}, or {@link NDIP5Receiver#RECEIVE_BANDWIDTH_HIGHEST}.
     ...
     * The source can be disconnected or changed later without recreating the instance.
     *
     * The receiver will receive the {@link ColorFormat#UYVY_BGRA} color format, using the {@link NDIP5Receiver#RECEIVE_BANDWIDTH_HIGHEST} bandwidth,
     * allows fielded video, and uses a generated name.
     *
     * @param source The source to attach the receiver to.
     */
    public NDIP5Receiver(NDIP5Source source) {
     ...
     */
    public void connect(NDIP5Source source) {
     ...
     *
     * This will return {@link NDIP5FrameType#NONE} if no data is received in the timeout span.
     * This will return {@link NDIP5FrameType#ERROR} if the connection is lost.
     *
     * Frames that are filled with data MUST be later manually cleared, with calling {@link NDIP5VideoFrame#freeBuffer()}, {@link NDIP5VideoFrame#close}, or with a try-with-resources.
     *
     ...
     *
     * @return The frame type that was captured, {@link NDIP5FrameType#NONE}, or {@link NDIP5FrameType#ERROR}
     */
    public NDIP5FrameType receiveCapture(NDIP5VideoFrame videoFrame, NDIP5AudioFrame audioFrame, NDIP5MetadataFrame metadataFrame, int timeout) {
        if(videoFrame != null) {
        ...
        if(frameType == NDIP5FrameType.VIDEO && videoFrame != null) {
            videoFrame.allocatedBufferSource.set(this);
        } else if (frameType == NDIP5FrameType.AUDIO && audioFrame != null) {
            audioFrame.allocatedBufferSource.set(this);
        } else if (frameType == NDIP5FrameType.METADATA && metadataFrame != null) {
     ...
     */
    public boolean sendMetadata(NDIP5MetadataFrame metadataFrame) {
     ...
     *
     * @param tally The {@link NDIP5Tally} state to send.
     * @return true if a source is connected, false if the receiver is disconnected.
     */
    public boolean setTally(NDIP5Tally tally) {
        return receiveSetTally(ndilibRecievePointer, tally.isOnProgram(), tally.isOnPreview());
    }

    /**
     * Fills in a {@link NDIP5PerformanceData} structure with performance information about the receiver.
     ...
     */
    public void queryPerformance(NDIP5PerformanceData performanceData) {
     ...
     * To avoid the need to poll this function, you can monitor {@link NDIP5Receiver#receiveCapture(NDIP5VideoFrame, NDIP5AudioFrame, NDIP5MetadataFrame, int)} for a {@link NDIP5FrameType#STATUS_CHANGE}
     *
    ...
    @Override
    void freeVideo(NDIP5VideoFrame frame) {
        freeVideoV2(ndilibRecievePointer, frame.structPointer);
    }

    @Override
    void freeAudio(NDIP5AudioFrame frame) {
        freeAudioV2(ndilibRecievePointer, frame.structPointer);
    }

    @Override
    public void freeMetadata(NDIP5MetadataFrame frame) {
