...
public class NDIP5VideoFrame implements AutoCloseable {

    final long structPointer;

    // set when a buffer is allocated by a receiver that later needs to be freed w/ that receiver.
    AtomicReference<NDIP5FrameCleaner> allocatedBufferSource = new AtomicReference<>();
    ...
    public void setFourCCType(NDIP5FrameFourCCType type) {
        setFourCCType(structPointer, type.id);
    }
    public NDIP5FrameFourCCType getFourCCType() {
        return NDIP5FrameFourCCType.valueOf(getFourCCType(structPointer));
    }
    ...
    public void setFormatType(NDIP5FrameFormatType type) {
        setFrameFormatType(structPointer, type.id);
    }
    public NDIP5FrameFormatType getFormatType() {
        return NDIP5FrameFormatType.valueOf(getFrameFormatType(structPointer));
    }
